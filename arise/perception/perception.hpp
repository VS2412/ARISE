...
// Always-on perception loop. Spawns three internal threads:
...
//
// Heavy ML pieces (Moondream caption, YAMNet audio scene) are deliberately
// not here — they hook in via blackboard subscribers in Phase 2 commit 2 so
// the scaffold stays runnable with just grim + niri installed.
//
// Topics published:
...
public:
    struct Config {
        ...
        MemoryCortex*  cortex = nullptr;     // optional — if set, screen_changed
                                             // events also write episodic rows
